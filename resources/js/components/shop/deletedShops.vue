                        ...
                        >
                         <td>{{ shop.id }}</td>
                            <td>
                                 ...
                                 <button @click="restoreShop(shop.id)" class="btn btn-info mx-2"><i class="fas fa-recycle"></i></button>
...
methods: {
    getData(){
        axios.get('/api/shops/deleted').then(resp =>{
    ...
    deleteShop(id){
        axios.delete('/api/shops/'+id).then(resp=>{
            console.log(resp)
     ...
     restoreShop(id){
        axios.get('/api/shops/undelete/'+id).then(resp=>{
            if(resp.data.status == 1){
                $('.tr'+id).fadeOut();
